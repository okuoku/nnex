...
(library (nnex scheme match)
  ...
  (import
   (only (scheme base) >= reverse cons vector-length = + vector-ref define-syntax syntax-rules _ ... vector? define if let pair? let-syntax begin car cdr lambda error quote null? equal? and or list? quasiquote unquote unquote-splicing => cond else)
   )
